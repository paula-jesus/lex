...
mask = dados_metas_planilha['BASE'] == 'Ag'
dados_metas_planilha = dados_metas_planilha[mask]

mask = dados_pesos['BASE'] == 'Ag'
...
for column in ['Auditoria', 'Auto avaliação', 'Aderência ao Plano de Capacitação da Qualidade definido para a Base', 'Custo / Pacote']:
    dados_planilha[column] = pd.to_numeric(dados_planilha[column], errors='coerce')

for column in ['Auditoria', 'Auto avaliação','Atendimento ao SLA (%) Agências', 'OPAV', 'Produtividade', 'Loss Rate ($)', 'Absenteísmo (%)', 'Aderência ao Plano de Capacitação da Qualidade definido para a Base', 'Inventario (%) Agências', 'Custo/ Pacote ($) Agências', 'Total de ocorrências de +2HE', 'Total de ocorrências de -11Hs Interjornadas']:
...
dados_pesos[[ 'OPAV', 'Absenteísmo (%)', 'Aderência ao Plano de Capacitação da Qualidade definido para a Base', 'Inventario (%) Agências', 'Custo/ Pacote ($) Agências', 'Total de ocorrências de +2HE', 'Total de ocorrências de -11Hs Interjornadas', 'Auditoria','Auto avaliação', 'Produtividade', 'Atendimento ao SLA (%) Agências', 'Loss Rate ($)']] = dados_pesos[[ 'OPAV', 'Absenteísmo (%)', 'Aderência ao Plano de Capacitação da Qualidade definido para a Base', 'Inventario (%) Agências', 'Custo/ Pacote ($) Agências', 'Total de ocorrências de +2HE', 'Total de ocorrências de -11Hs Interjornadas', 'Auditoria','Auto avaliação', 'Produtividade', 'Atendimento ao SLA (%) Agências', 'Loss Rate ($)']].apply(lambda x: x*100)
...
dados_compilados.rename(columns={'month': 'Mês', 'opav': 'OPAV', 'produtividade_media': 'Produtividade Média', 'loss_rate': 'Loss Rate','Auto avaliacao':'Auto avaliação','sla':'SLA','two_hrs':'Ocorrências de +2HE','abs':'Absenteísmo','cnt_interjornada':'Ocorrências de -11Hs Interjornadas', 'percent_inventoried': 'Inventário' }, inplace=True)
...
dados_metas_planilha.rename(columns={'Absenteísmo (%)': 'Absenteísmo','Total de ocorrências de +2HE': 'Ocorrências de +2HE', 'Total de ocorrências de -11Hs Interjornadas':'Ocorrências de -11Hs Interjornadas', 'Produtividade':'Produtividade Média', 'Atendimento ao SLA (%) Agências': 'SLA', 'Inventario (%) Agências': 'Inventário', 'Loss Rate ($)': 'Loss Rate', 'Custo/ Pacote ($) Agências': 'Custo / Pacote'}, inplace=True)
...
row_labels = ['OPAV', 'Absenteísmo','Auditoria','Auto avaliação', 'Inventário', 'Aderência ao Plano de Capacitação da Qualidade definido para a Base', 'SLA']  
...
row_labels = ['Ocorrências de +2HE','Ocorrências de -11Hs Interjornadas','Produtividade Média']  
...
with tab1:
    ...
    meta_value_percent = str(dados_metas_aderencia['meta'].median())
    ...
    fig = go.Figure(
        ...
        go.Indicator(
            ...
            gauge={
                ...
                'bar': {'color': '#8fe1ff'},
                'steps': [
                    {'range': [0, 90], 'color': '#fab8a3'},  
                    {'range': [90, 100], 'color': '#c9f1ac'},  
                ],
                'threshold': {
    ...
    meta_value_percent = str(dados_metas_abs['meta'].median())
        ...
        go.Indicator(
            ...
            gauge={
                ...
                'steps': [
                    ...
                    {'range': [meta_value_decimal, 10], 'color': '#fab8a3'},  
                ],
                'threshold': {
                    'line': {'color': "black", 'width': 4},
                    'thickness': 0.75,
...
with tab2:
    ...
    meta_value_percent = str(dados_metas_opav['meta'].median())
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            gauge={
                ...
                'steps': [
                    {'range': [0, meta_value_decimal], 'color': '#c9f1ac'},
                    {'range': [meta_value_decimal, 100], 'color': '#fab8a3'}
                ],
                'threshold': {
                    'line': {'color': "black", 'width': 4},
                    'thickness': 0.75,
...
with tab3:
    ...
    dados_metas_sla = dados_metas_planilha[['SLA']].rename(columns={'SLA': 'meta'})

    meta_value_decimal = dados_metas_sla['meta'].median()

    meta_value_percent = str(dados_metas_sla['meta'].median())

    meta_value_percent = meta_value_percent.rstrip('0').rstrip('.') if '.' in meta_value_percent else meta_value_percent

    col1.plotly_chart(create_area_plot(dados_lex_resampled, "SLA", 'SLA', f"Meta: {meta_value_percent}%", meta_value_decimal, meta_value_decimal))
        ...
        go.Indicator(
            ...
            gauge={
                ...
                'steps': [
                    ...
                    {'range': [meta_value_decimal, 100], 'color': '#c9f1ac'}, 
                ],
                'threshold': {
                    'line': {'color': "black", 'width': 4},
                    'thickness': 0.75,
